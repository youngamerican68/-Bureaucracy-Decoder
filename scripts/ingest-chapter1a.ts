...
/**
 * Ingest Chapter 1A Downtown Zoning Code into Supabase
 *
 * This script:
 * 1. Loads scraped Chapter 1A data (MARKDOWN FORMAT)
 * 2. Chunks the content using polymorphic parser (auto-detects markdown)
...
const DOC_SLUG = 'los-angeles-chapter1a';
const CITY_NAME = 'Los Angeles';
const CODE_NAME = 'Chapter 1A - Downtown Zoning';
const SOURCE_URL = 'https://codelibrary.amlegal.com/codes/los_angeles/latest/lamc/0-0-0-169651';
...
function splitOversizedChunk(chunk: RegulationChunk): ProcessedChunk[] {
  ...
  // Attempt to split by subsections (e.g., 1.3.1.1, 1.3.1.2)
  ...
  if (subsections.length >= 2) {
    ...
    for (let i = 0; i < subsections.length; i++) {
      ...
      if (subsectionTokens > MAX_TOKEN_HARD_LIMIT) {
        console.log(
          `    ⚠️  Subsection ${subsections[i].ref} still too large (${subsectionTokens} tokens). Truncating.`
        );
  ...
  console.log(
    `    ⚠️  Cannot split chunk ${chunk.section_ref}. Truncating to ${MAX_TOKEN_HARD_LIMIT} tokens.`
  );
...
async function main() {
  ...
  console.log('CHAPTER 1A DOWNTOWN ZONING CODE INGESTION');
  console.log('='.repeat(80));
  console.log();
  ...
  if (fetchError && fetchError.code === 'PGRST116') {
    ...
    await supabase.from('zoning_docs').update({ status: 'ingesting' }).eq('id', docId);
  ...
  console.log('Step 4: Chunking content (markdown format)...');
  const allChunks: RegulationChunk[] = [];

  for (const article of rawData) {
    const chunks = chunkMunicipalCode(article.content, article.url);
    allChunks.push(...chunks);

    // Extract article name from URL or content for logging
    const articleName = article.url.split('/').pop() || 'unknown';
    console.log(`  → Article ${articleName}: ${chunks.length} chunks`);
  }
  ...
  for (let i = 0; i < embeddingRecords.length; i += DB_INSERT_BATCH_SIZE) {
    ...
    const { error: insertError } = await supabase.from('zoning_embeddings').insert(batch);
  ...
  console.log(
    `Total tokens: ${embeddingRecords.reduce((sum, r) => sum + r.token_count, 0).toLocaleString()}`
  );
  console.log(
    `Average tokens/chunk: ${Math.round(embeddingRecords.reduce((sum, r) => sum + r.token_count, 0) / embeddingRecords.length)}`
  );
  console.log(`\nVerify in Supabase:`);
